import { CalcFeatureRow, CalcRequestBody } from './types.js';

function clamp(val: number, min = 0, max = 1) {
  return Math.max(min, Math.min(max, val));
}

export function evaluateActionsSimple(req: CalcRequestBody): CalcFeatureRow[] {
...
function estimateHazardDamagePercent(hazards: { stealthRock?: boolean; spikes?: number }, types: string[]): number {
  let percent = 0;
